...
There may be services which use other services which are all developed by us.

## Data-issues

The root-cause of a data-set-problem is the original reason why a wrong data-set comes into our system.
If possible also explain if data is corrupt permanently or if it is just a temporary problem or if there is an auto-heal-mechanism which will fix the data automatically.
...
Unless other information is given, assume that it was already working before and some change (in source-code or in data) introduced the bug.
